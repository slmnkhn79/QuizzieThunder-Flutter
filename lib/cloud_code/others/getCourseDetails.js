const getCourseDetails = async (request) => {
    ...
    const moduleDetails = await Promise.all(modules.map(async (module) => {
        ...
        if (milestone) {
            ...
            UserProgressQuery.equalTo('completedModule', module);

            // UserProgressQuery.equalTo('user',USER);
            const userProgress = await UserProgressQuery.find();
            // var ifExists = false;
            // ifExists = userProgress.map((e) => {
            //     if (e.relation('completedModule').query().equalTo('objectId', milestone.id))
            //         return true;
            // });
