...
Parse.Cloud.define("profile", async (request) => {
  ...
  try {
    ...
    const userDetail = {
      ...
      'quizwon': parseUserDetail.get('quiz_won') ?? 0,
      'rank': parseUserDetail.get('rank') ?? 0,
      'quizAttempted': parseUserDetail.get('quizAttempted') ?? 0,
    }
...
Parse.Cloud.define('login', async (request) => {
  ...
  try {
    ...
    throw new Parse.Error(Parse.Error.OBJECT_NOT_FOUND, "Invalid username or password. error.message");
  }
});
...
Parse.Cloud.define("videoById", async (request) => {
  ...
  try {
    const videosQuery = new Parse.Query(MentorVideos);
    const videos = await videosQuery.get(videoId);
    return {
      status: true,
      code: 200,
      message: "",
...
Parse.Cloud.define("updateProfile", async (request) => {
  ...
  try {
    ...
    if (about) {
      user.set('description', about.trim());
    }
    else {
...
Parse.Cloud.define("requestPasswordReset", async (request) => {
  ...
  try {
    ...
    if (user) {
      ...
      if (result) {
        return { code: 200, status: true, message: "Password reset request was sent successfully." };
      }
...
Parse.Cloud.define('decryptData', async (request) => {
  try {
    // Base64 encoded encrypted data received as request parameter
    const base64EncryptedData = request.params.encryptedMessage;

    // Replace with your own private key, possibly load from environment variable
    const privateKey = `
    -----BEGIN RSA PRIVATE KEY-----

XGel9fwz1rKCsnJLXZ57huptJHSx6j8rIIILaXAbWd/0
-----END RSA PRIVATE KEY-----
    `;
    ...
    // Convert buffer to string (decrypted plaintext)
    const decryptedText = decryptedData.toString('utf8');

    return { "decryptedText": decryptedText }; // Returning the decrypted text
  } catch (error) {
