...
Parse.Cloud.define("signUpData", async (request) => {
  try {
    ...
    if (!firstname || !lastname || !email      || !dob      || !state      || !country      || !username      || !password      || !confirmPassword      || !age      || !gender      || !interest      || !skill || !summary) 
    {
    ...
    return { code: 400, status:false, "message": e.toString() }
  }
});
