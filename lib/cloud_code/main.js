...
Parse.Cloud.define("learning", async (request) => {
  const LearningPaths = Parse.Object.extend("LearningPaths");
  const News = Parse.Object.extend("News");
  const InterWork = Parse.Object.extend("Inter_work");
  const MentorVideos = Parse.Object.extend("MentorVideos");
  const Books = Parse.Object.extend("Books");


  try {
    const learningPathQuery = new Parse.Query(LearningPaths);
    const paths = await LearningPaths.find();

    const newsQuery = new Parse.Query(News);
    newsQuery.limit(10);
    const news = await newsQuery.find();

    const intWorkQuery = new Parse.Query(InterWork);
    intWorkQuery.limit(10);
    const intWork = await intWorkQuery.find();

    const mentorVideosQuery = new Parse.Query(MentorVideos);
    mentorVideosQuery.limit(10);
    const mentorVideos = await mentorVideosQuery.find();

    const booksQuery = new Parse.Query(Books);
    booksQuery.limit(10);
    const books = booksQuery.find();
    ...
    return {
      ...
      data: {
        ...
        news: news,
        intWork : intWork,
        mentorVideos: mentorVideos,
        books : books
      }
    }
 }
  catch (error) {
    throw new Parse.Error(Parse.Error.OBJECT_NOT_FOUND, "Failed to fetch learning data");
  }
});