...
class WondersTimelineBuilder extends StatelessWidget {
  ...
  final List<dynamic> selectedWonders;

  final Widget Function(BuildContext, 
  // WonderData 
  dynamic type, bool isSelected)? timelineBuilder;
  final Axis axis;
  ...
  Widget build(BuildContext context) {
    final gap = crossAxisGap ?? AppStyle().insets.xs;
    ...
    return LayoutBuilder(builder: (_, constraints) {
      ...
      Widget buildSingleTimelineTrack(BuildContext context, List<
      dynamic // WonderType 
      > types) {
        return Stack(
          ...
          children: types.map(
            (t) {
              return Positioned(left: 0, width: 600, top: 0, bottom: 0, child: Image.asset('images/placeholder.png'));
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        color: isSelected ? AppStyle().colors.accent2 : Colors.transparent,
        borderRadius: BorderRadius.circular(99),
        border: Border.all(color: AppStyle().colors.accent2),
      ),
    );
  }
}
